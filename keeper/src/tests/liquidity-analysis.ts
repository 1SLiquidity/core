...
export async function analyzeTokenPairLiquidityComprehensive(
  ...
  try {
    ...
    const dexResults = allReserves.map(reserve => ({
      ...
      totalLiquidity: weiToNormal(reserve.reserves.token0, reserve.decimals.token0) + 
    ...
    const totalLiquidityUSD = dexResults.reduce((sum, dex) => sum + dex.totalLiquidity, 0)
    const highestLiquidity = Math.max(...dexResults.map(d => d.totalLiquidity))
    ...
    dexResults.forEach(dex => {
      console.log(`  ${dex.name}: ${dex.reservesNormal.tokenA.toFixed(6)} ${tokenAInfo.symbol} / ${dex.reservesNormal.tokenB.toFixed(6)} ${tokenBInfo.symbol} (Total: ${dex.totalLiquidity.toFixed(6)})`)
    })
    console.log(`\nTotal Reserves: ${weiToNormal(totalReservesA.toString(), tokenAInfo.decimals).toFixed(6)} ${tokenAInfo.symbol} / ${weiToNormal(totalReservesB.toString(), tokenBInfo.decimals).toFixed(6)} ${tokenBInfo.symbol}`)
    console.log(`\nSweet Spot: ${sweetSpot} streams`)
    console.log(`\nSlippage Analysis (${bestDex.name}):`)
    console.log(`  Slippage Savings: ${slippageSavings.toFixed(6)} ${tokenBInfo.symbol}`)
