...
export class CurveService {
  ...
  async getReserves(tokenA: string, tokenB: string): Promise<ReserveResult | null> {
    try {
      ...
      const [tokenAIndex, tokenBIndex] = await this.getTokenIndices(tokenA, tokenB);
  ...
  async getPrice(tokenA: string, tokenB: string): Promise<PriceResult | null> {
    try {
      ...
      const amountOut = await this.pool.get_dy(tokenAIndex, tokenBIndex, amountIn);
   ...
   */
  private async getTokenIndices(tokenA: string, tokenB: string): Promise<[number, number]> {
    try {
