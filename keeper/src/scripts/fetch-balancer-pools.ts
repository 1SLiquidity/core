...
async function fetchAllBalancerPools(): Promise<BalancerPoolInfo[]> {
  ...
  if (!subgraphUrl) {
    throw new Error('BALANCER_SUBGRAPH environment variable is required. Please set it in your .env file.');
  }
  ...
  while (hasMore) {
    ...
    console.log(`Received ${batchPools.length} pools in batch starting at ${skip}`);
    ...
    for (const pool of batchPools) {
      try {
        ...
        if (poolInfo) {
          pools.push(poolInfo);
          console.log(`✓ ${poolInfo.symbol} pool (${poolInfo.poolAddress}) - ${poolInfo.tokens.length} tokens`);
        }
      } catch (error) {
        console.log(`✗ Failed to process pool ${pool.address}:`, (error as Error).message);
      }
    }
...
async function main() {
  try {
    ...
    fs.writeFileSync(jsonPath, JSON.stringify(pools, null, 2));
...
export { fetchAllBalancerPools, generateDexConfig };
