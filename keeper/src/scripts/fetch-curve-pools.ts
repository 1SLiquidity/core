  ...
  while (retries <= maxRetries) {
    try {
      ...
      if (retries === maxRetries) {
        console.log(`❌ ${name} failed after ${maxRetries} retries:`, (error as Error).message);
        return null;
      }
      
      const delay = baseDelay * Math.pow(2, retries); // Exponential backoff
      console.log(`⚠️  ${name} failed (attempt ${retries + 1}/${maxRetries + 1}). Retrying in ${delay}ms...`);
...
async function fetchAllCurvePools(): Promise<CurvePoolInfo[]> {
  ...
  for (let i = 0; i < Number(poolCount); i += batchSize) {
    ...
    if (!poolAddresses) {
      console.log(`Skipping batch ${i}-${endIndex - 1} due to repeated failures`);
      continue;
    }
    ...
    for (let k = 0; k < poolAddresses.length; k++) {
      ...
      try {
        ...
        console.log(`✗ Failed to fetch pool ${poolAddress}:`, (error as Error).message);
      }
    ...
    console.log(`Completed batch ${i}-${endIndex - 1}. Waiting ${requestDelay}ms before next batch...`);
...
async function fetchPoolInfo(
  ...
  try {
    ...
    const underlyingTokens = underlyingCoins.filter((coin: string) => coin !== ethers.ZeroAddress);
    ...
    console.log(`Error fetching pool info for ${poolAddress}:`, (error as Error).message);
    return null;
  }
}
...
function generateDexConfig(pools: CurvePoolInfo[]): string {
  ...
  filteredOut.forEach(pool => {
    console.log(`  - ${pool.poolName} (${pool.poolAddress}): ${pool.tokens.length} tokens`);
  });
...
// Total pools: ${activePools.length} (${regularPools.length} regular, ${metaPools.length} meta)
...
export const CURVE_POOL_METADATA = {`;
  ...
  activePools.forEach(pool => {
    ...
    config += `    underlyingTokens: [${pool.underlyingTokens.map(t => `'${t}'`).join(', ')}],\n`;
    config += `    A: '${pool.A}',\n`;
    config += `    fee: '${pool.fee}',\n`;
...
export { fetchAllCurvePools, generateDexConfig };
