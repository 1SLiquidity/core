...
class DEXService {
    ...
    async getReserves(tokenA, tokenB) {
        ...
        // Uniswap V2
        try {
            const uniswapV2Factory = new ethers_1.ethers.Contract(DEX_ADDRESSES.UNISWAP_V2.FACTORY, FACTORY_ABIS.UNISWAP_V2, this.provider);
            const pairAddress = await uniswapV2Factory.getPair(tokenA, tokenB);
            if (pairAddress !== ethers_1.ethers.ZeroAddress) {
                const pair = new ethers_1.ethers.Contract(pairAddress, PAIR_ABIS.UNISWAP_V2, this.provider);
        ...
        // Uniswap V3
        try {
            const uniswapV3Factory = new ethers_1.ethers.Contract(DEX_ADDRESSES.UNISWAP_V3.FACTORY, FACTORY_ABIS.UNISWAP_V3, this.provider);
            ...
            for (const fee of feeTiers) {
                ...
                if (poolAddress !== ethers_1.ethers.ZeroAddress) {
                    const pool = new ethers_1.ethers.Contract(poolAddress, PAIR_ABIS.UNISWAP_V3, this.provider);
