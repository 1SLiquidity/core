...
async function main() {
    ...
    if (state.lastScannedBlock === 0n)
        state.lastScannedBlock = CONFIG.deploymentBlock;
    console.log(`[init] cold scan from ${state.lastScannedBlock}…`);
    await coldScan(http, CONFIG.contractAddress, CONFIG.deploymentBlock, state, store, CONFIG.logChunk);
    const executor = new Executor(wallet, CONFIG.contractAddress, CONFIG.gasMultiplier, CONFIG.maxParallelTx);
    if (mode === 'once') {
    ...
    if (mode === 'ws') {
        ...
        if (!CONFIG.wsUrl)
            throw new Error('WS_URL required for ws mode');
        ...
        setInterval(() => executor.executeOutstanding(state, store).catch((err) => console.error('[tick]', err)), 10_000);
        return;
    }
