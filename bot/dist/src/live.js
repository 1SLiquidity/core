...
export function attachLiveListeners(ws, address, state, store, confirmations) {
    ...
    ws.on({ address }, async (log) => {
        ...
        if (head - (log.blockNumber ?? head) < confirmations)
            return;
        try {
            ...
            if (ev.name === 'TradeCreated') {
                const tokenIn = ev.args.tokenIn;
                const tokenOut = ev.args.tokenOut;
                const pairId = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(['address', 'address'], [tokenIn, tokenOut]));
                ...
                if (completed)
                    state.pairs[pairId].trades[tradeId].completed = true;
                store.save(state);
            }
